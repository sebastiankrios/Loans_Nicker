...
while True:
    ...
    if opcion in OPCIONES_VALIDAS:
        match opcion:
                        ...
                        if funciones.ValidarNombreApellido(apellido):
                            if funciones.ValidarCorreo(correo):
                                if funciones.ValidarTiempoPrestamo(dias_pasar) == True: 
                                    ...
                                    print('Cordial saludo, querido prestamista, el tiempo ingresado no cumple con las reglas, paila')
                                    continue
                            else:
                                print('Cordial saludo, querido prestamista, su correo no cumple con las reglas, paila')
                                continue
                        else:
                            print('Cordial saludo, querido prestamista, su apellido no cumple con las reglas, paila')
                            continue
                    else:
                        print('Cordial saludo, querido prestamista, su nombre no cumple con las reglas, paila')
                        continue
                else:
                    print('Cordial saludo, querido prestamista, su cedula no cumple con las reglas, paila')
                    continue
                        ...
                        if funciones.ValidarCategoria(categoria_item):
                            if funciones.ValidarPrecio(precio_item):
                                ...
                                if estado_texto != False:
                                    ...
                                    print(f" ID generado automaticamente para el item: {id_item_generado}")
                                    inventario[id_item_generado] = [nombre_item, categoria_item, precio_item, estado_texto]
                                    consecutivo_items += 1
                                    print('item registrado en el almacenn con exito!')
                                else:
                                    print('Cordial saludo, querido prestamista, la calificacion del estado no cumple con las reglas, paila')
                            else:
                                print('Cordial saludo, el precio ingresado no cumple con las reglas, paila')
                        else:
                            print('Cordial saludo, la categoriaa ingresada no cumple con las reglas, paila')
                    else:
                        print('Cordial saludo, el nombre del item no cumple con las reglas, paila')    
                elif sub_opcion == '2':
                    funciones.RegistrarPrestamo(usuarios, inventario, prestamos)
                else:
                    print('Opcion no disponible en el submenu. Volviendo al menu principal.')
            ...
            case '6':
                print('\tAdministrador')
                user_ingresado = input("Favor ingresar el usuario administrador: ")
                ...
                if es_admin == True:
                    ...
                    print("\nError: Usuario o contraseña incorrectos. Acceso denegado, volviendo al menu.")
            
            case '7':
                print('Gracias por usar Loans_Nicker')
                break
